...
public:
    ...
    //客户端消息事件
    virtual void OnNetMsg(LeptServer* pLeptServer, ClientSocketPtr& pClient, DataHeader* header) = 0;
    //recv事件
...
public:
    LeptS2LTask(ClientSocketPtr& pClient, DataHeaderPtr& header)
    {
        _pClient = pClient;
        _pHeader = header;
    }
...
public:
    ...
    SOCKET Accept()
    {
        ...
        else
        {
            ...
            //addClientToLeptServer(std::make_shared<ClientSocket>(cSock));
    ...
    void addClientToLeptServer(ClientSocketPtr& pClient)
    {
    ...
    //LeptServer 4 多个线程触发 不安全
    //如果只开启1个LeptServer就是安全的
    virtual void OnNetMsg(LeptServer* pLeptServer, ClientSocketPtr& pClient, DataHeader* header)
    {
        _msgCount++;
    }
